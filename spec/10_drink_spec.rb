...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:drink_type) { described_class.new('tea') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(drink_type.type).to eq('tea')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(drink_type.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:full_drink) { described_class.new('tea') }
      # remove the 'x' before running this test
      it 'is full' do
        expect(full_drink).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:half_drink) { described_class.new('tea', 10) }

      # remove the 'x' before running this test
      it 'is not full' do
      expect(half_drink).not_to be_full
      end
    end
  end
end
