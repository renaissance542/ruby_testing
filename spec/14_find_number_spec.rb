...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:game_over_five) { described_class.new(5, 9, number_guessing) }

      it 'returns 7' do
        expect(game_over_five.make_guess).to eq(7)
      end
    end
    ...
    context 'when min is 8 and max is 9' do
      ...
      it 'returns 8' do
        expect(game_guess_eight.make_guess).to eq(8)
      end
    end
    ...
    context 'when min is 0 and max is 3' do
      ...
      it 'returns 1' do
        expect(game_guess_one.make_guess).to eq(1)
      end
    end
    ...
    context 'when min and max both equal 3' do
      ...
      it 'returns 3' do
        expect(game_guess_three.make_guess).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:number_eight) { double('number_eight', value: 8) }
      subject(:game_correct_guess) { described_class.new(0, 9, number_eight, 8) }
    ...
    context 'when guess and random_number are not equal' do
      let(:number_eight) { double('number_eight', value: 8) }
      subject(:game_wrong_guess) { described_class.new(0, 9, number_eight, 6) }
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      subject(:one_value_left) { described_class.new(5, 8, number_range, 7) }

      before do
        one_value_left.update_range
      end

      it 'updates min to the same value as max' do
        expect(one_value_left.min).to eq(8)
      end

      it 'does not update max' do
        expect(one_value_left.max).to eq(8)
      end
    end

    context 'when the guess is the answer' do
      subject(:guessed_the_answer) { described_class.new(2, 9, number_range, 8) }

      before do
        guessed_the_answer.update_range
      end

      it 'does not update the range' do
        expect(guessed_the_answer.min).to eq(2)
        expect(guessed_the_answer.max).to eq(9)
      end
    end
  end
end
