...
describe NumberGame do
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        bad_input = 'g'
        valid_input = '4'
        allow(game_loop).to receive(:player_input).and_return(bad_input, bad_input, valid_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      subject(:game_three)  { described_class.new(4, '4', 3) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
    ...
    context 'when count is 4 and over' do
      subject(:game_six) { described_class.new(4, '4', 6) }

      it 'outputs correct phrase' do
        correct_phrase = "That was hard. It took you 6 guesses!\n"
        expect { game_six.final_message }.to output(correct_phrase).to_stdout
      end
    end
  end
end
