...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        decimal = '3.42'
        valid_input = '3'
        allow(game_input).to receive(:gets).and_return(letter, decimal, valid_input)
      end
  ...
  describe '#verify_input' do
    subject(:game_verify) { described_class.new(1, 9) }
    ...
    context 'when given a valid input as argument' do
      ...
      it 'returns valid input' do
        ...
        valid_input = '4'
        expect(game_verify.verify_input(min, max, valid_input.to_i)).to eq(valid_input.to_i)
      end
    end
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        max = game_verify.instance_variable_get(:@maximum)
        letter = 'm'
        expect(game_verify.verify_input(min, max, letter.to_i)).to be_nil
      end
    end
  end
  ...
  describe '#maximum_guesses' do
    ...
    context 'when game minimum and maximum is 100 and 600' do
      subject(:game_5hundred) { described_class.new(100, 600) }

      it 'returns 9' do
        expect(game_5hundred.maximum_guesses).to eq(9)
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    subject(:game_turn_order) { described_class.new(1, 9) }
    let(:turn_order_search) { instance_double(BinarySearch) }

    before do
      # You'll need to create a few method stubs.
      allow(turn_order_search).to receive(:make_guess)
      allow(turn_order_search).to receive(:update_range)
      allow(game_turn_order).to receive(:display_guess).with(turn_order_search)
      allow(turn_order_search).to receive(:game_over?)
    end
    
    
    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      game_turn_order.display_turn_order(turn_order_search)
      expect(game_turn_order.instance_variable_get(:@guess_count)).to eq(1)
    end
    
    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(turn_order_search).to receive(:make_guess).once
      game_turn_order.display_turn_order(turn_order_search)
    end
    
    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(turn_order_search).to receive(:update_range).once
      game_turn_order.display_turn_order(turn_order_search)
    end
